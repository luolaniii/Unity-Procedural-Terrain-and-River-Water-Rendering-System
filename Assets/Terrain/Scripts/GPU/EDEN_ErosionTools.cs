...
{
    public class EDEN_ErosionTools : MonoBehaviour
    {
        [Header("Hydraulics")]
        ...
        public void HydraulicErode()
        {
            ...
            {
                heightMap = EDEN_Hydraulics.Erode(HydraulicErosionComputeShader, iterations, heightMap, heightmapResolution, resolution + 1);
        ...
        public void WindErode()
        {
            ...
            heightMap = EDEN_Wind.Erode(WindErosionComputeShader, (int)(600000 * Windiness), heightMap, heightmapResolution, w_Resolution, WindDirection);
        ...
        public void Terrace()
        {
            ...
            TerraceComputeShader.SetTexture(TerraceComputeShader.FindKernel("Terrace"), "normalHeightmap", heightmapTex);
        ...
        public void Smooth(int width)
        {
            ...
            SmoothSharpenComputeShader.SetTexture(SmoothSharpenComputeShader.FindKernel("Smooth"), "heightMap", heightmapTex);
            SmoothSharpenComputeShader.SetInt("heightSize", heightmapResolution);
            SmoothSharpenComputeShader.SetInt("sWidth", SmoothingWidth);
            SmoothSharpenComputeShader.Dispatch(SmoothSharpenComputeShader.FindKernel("Smooth"), rt.width / 8, rt.height / 8, 1);
        ...
        public void Sharpen()
        {
            ...
            for (int i = 0; i < sharpenIterations; i++)
            {
                ...
                SmoothSharpenComputeShader.SetTexture(SmoothSharpenComputeShader.FindKernel("Sharpen"), "heightMap", heightmapTex);
                SmoothSharpenComputeShader.SetInt("heightSize", heightmapResolution);
                SmoothSharpenComputeShader.SetFloat("shstr", SharpenStrength);
                SmoothSharpenComputeShader.SetFloat("pmstr", PeakMixStrength);
                SmoothSharpenComputeShader.Dispatch(SmoothSharpenComputeShader.FindKernel("Sharpen"), rt.width / 8, rt.height / 8, 1);
        ...
        void ReadRenderTextureToCPU(int heightmapResolution)
        {
            ...
            for (int x = 0; x < heightmapResolution; x++)
            {
                ...
                {
                    heightMap[x, y] = rtTex2d.GetPixel((int)(((float)x / heightmapResolution) * rtTex2d.width), (int)(((float)y / heightmapResolution) * rtTex2d.height)).r;
                }
            }
        }
        ...
        Texture2D ConvertHeightmapToTexture(int heightmapResolution)
        {
            ...
            for (int x = 0; x < heightmapResolution; x++)
            {
                for (int y = 0; y < heightmapResolution; y++)
                {
                    tex.SetPixel((int)(((float)x / heightmapResolution) * tex.width), (int)(((float)y / heightmapResolution) * tex.height), Color.white * heightMap[x, y]);
                }
            }
